...
const Followed = () => {
    ...
    const morePodcasts = () => {
        ...
        PodcastService.getFollowedPodcast(authContext.getAuthHeader(), page).then(result => {
    ...
    useEffect(()=> {
        if(authContext.getAuthHeader())
        PodcastService.getFollowedPodcast(authContext.getAuthHeader(), 1, searchContext.searchText).then(result => {
            setPodcasts(result)
            setIsLoading(false)
        })
    }, [authContext, searchContext.searchText])
    ...
    return (
        <React.Fragment>
            ...
            {(podcasts.length === 0 && !isLoading) && <h2 className='text-center mt-5 fw-bold'>No sigues a ningun creador.</h2>}
